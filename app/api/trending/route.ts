...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Fetching tokens: type=${type}, timeframe=${timeframe}, limit=${limit}, search=${search}, timeRange=${startTime} to ${endTime}`)
    ...
    if (search && search.length > 2) {
      ...
      if (tokens.length === 0) {
        ...
        const trendingTokens = await getTrendingTokens(timeframe, limit * 3, timeRange)
      ...
      if (type === 'dex') {
        tokens = await getDexTokens(timeframe, limit, timeRange)
      } else if (type === 'new') {
        tokens = await getNewTokens(timeframe, limit, timeRange)
      } else {
        tokens = await getTrendingTokens(timeframe, limit, timeRange)
      }
    }
...
async function getTrendingTokens(timeframe: string, limit: number, timeRange?: { start: Date; end: Date }) {
  try {
    console.log('Fetching real trending tokens from Moralis API')
...
async function getDexTokens(timeframe: string, limit: number, timeRange?: { start: Date; end: Date }) {
  try {
    console.log('Getting DEX tokens...')
    
    // For now, return the same data as trending but mark them as DEX tokens
    const trendingTokens = await getTrendingTokens(timeframe, limit, timeRange)
...
async function getNewTokens(timeframe: string, limit: number, timeRange?: { start: Date; end: Date }) {
  try {
...
async function getTrendingFromMoralis(timeframe: string, limit: number) {
  try {
    console.log('Fetching real transaction data from Moralis Solana API...')
    
    if (!MORALIS_API_KEY) {
      throw new Error('Moralis API key not available')
    }

    // Calculate time range for the timeframe
    const now = new Date()
    const timeRanges = {
    ...
    const fromDate = timeRanges[timeframe as keyof typeof timeRanges] || timeRanges['24h']
    
    // Fetch top tokens by volume from Moralis Solana API
    const response = await fetch(`https://api.moralis.io/v2/solana/tokens/top?chain=solana&limit=${limit}&from_date=${fromDate.toISOString()}&to_date=${now.toISOString()}&order=volume_desc`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
    ...
    console.log(`Found ${tokens.length} tokens from Moralis Solana API`)

    if (tokens.length > 0) {
      // Enhance tokens with real transaction analytics
      const enhancedTokens = await Promise.all(tokens.slice(0, limit).map(async (token: any) => {
        try {
          // Get detailed analytics for this token
          const analyticsResponse = await fetch(`https://api.moralis.io/v2/solana/tokens/${token.address}/analytics?chain=solana&from_date=${fromDate.toISOString()}&to_date=${now.toISOString()}`, {
            headers: {
              'X-API-Key': MORALIS_API_KEY,
              'Content-Type': 'application/json'
            }
          ...
          return {
            ...
            name: token.name || 'Unknown Token',
            image: token.logo || `/tokens/default.png`,
            marketCap: Math.floor(token.market_cap || 0),
            liquidity: Math.floor(token.liquidity || 0),
            volume: Math.floor(token.volume_usd || 0),
            txns: analytics.transaction_count || 0,
            price: parseFloat(token.price || 0),
            change: parseFloat((token.percent_change_24h || 0).toFixed(2)),
            holders: token.holder_count || 0,
            age: token.age || 1,
            isPaid: false, // Moralis doesn't provide DEX paid status
            risk: risk,
            buyTxns: Math.floor((analytics.transaction_count || 0) * 0.6),
            sellTxns: Math.floor((analytics.transaction_count || 0) * 0.4),
            holderChange: token.holderChange24h ? `${token.holderChange24h > 0 ? '+' : ''}${token.holderChange24h.toFixed(1)}%` : '0.0%',
            // Add real transaction data
            transaction_count: analytics.transaction_count || 0,
            active_buyers: analytics.active_buyers || 0,
            active_sellers: analytics.active_sellers || 0,
            volume_usd: token.volume_usd || 0,
            percent_change_24h: token.percent_change_24h || 0,
            market_cap: token.market_cap || 0
          }
        } catch (error) {
          console.error(`Error fetching analytics for token ${token.address}:`, error)
          return {
            ca: token.address,
            symbol: token.symbol || 'UNK',
            name: token.name || 'Unknown Token',
      ...
      console.log(`Enhanced ${enhancedTokens.length} tokens with real transaction data`)
  ...
  try {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        console.warn(`Failed endpoint ${endpoint}:`, error instanceof Error ? error.message : String(error))
        continue
      }
    }
